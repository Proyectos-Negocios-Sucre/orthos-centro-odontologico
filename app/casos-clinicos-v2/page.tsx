...
export default function ClinicalCasesPage() {
  ...
  const treatments = useMemo(() => {
    const unique = Array.from(new Set(cases.map((c) => c.treatment)))
    return unique.sort()
  }, [])
  ...
  const totalPages = Math.ceil(filteredCases.length / PAGINATION.SERVICES_PER_PAGE)
    ...
    <>
      ...
      <section className="relative py-12 md:py-20 bg-gradient-to-r from-primary to-secondary text-white">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Casos Clínicos</h1>
          <p className="text-lg md:text-xl text-white/90 max-w-2xl">
            Transformaciones reales que devuelven la confianza. Explora cómo nuestra precisión clínica y visión estética han cambiado la vida de nuestros pacientes.
          </p>
        </div>
      </section>

      {/* Cases Grid */}
      <section className="py-12 md:py-20 bg-light-bg dark:bg-gray-950">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Search and Filter */}
...
function CaseCard({ caseItem }: CaseCardProps) {
  const [displayMode, setDisplayMode] = useState<'before' | 'after' | 'both'>('both')
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow border dark:border-gray-700">
      ...
      <div className="relative h-96 bg-gray-900 overflow-hidden">
        {displayMode === 'both' ? (
          caseItem.imageBefore && caseItem.imageAfter ? (
            <div className="flex h-full">
              <div className="relative flex-1">
                <Image
                  src={caseItem.imageBefore}
                  alt="Antes"
                  fill
      ...
      {/* Content */}
      <div className="p-6">
        <h3 className="text-2xl font-bold text-primary dark:text-primary mb-2">{caseItem.name}</h3>
        <p className="text-primary text-sm font-semibold uppercase mb-4 tracking-wider">{caseItem.treatment}</p>
        ...
        <div className="bg-light-bg dark:bg-gray-700 p-4 rounded-lg mb-6">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">Especialista</p>
              <p className="text-sm font-semibold text-text-light dark:text-white">{caseItem.specialist}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400 uppercase font-semibold">Duración</p>
              <p className="text-sm font-semibold text-text-light dark:text-white">{caseItem.duration}</p>
            </div>
          </div>
        </div>
